...
def group_auc(pred, true, padding_indicator=PADDED_Y_VALUE):
    ...
    result = binary_auroc(pred.squeeze(), true.squeeze(),
                          num_tasks=pred.shape[0], weight=mask.squeeze())
    if pred.shape[0] == 1:
...
def mrr(y_pred, y_true, ats=None, padding_indicator=PADDED_Y_VALUE):
    ...
    indices = indices.type_as(values).unsqueeze(
        dim=0).t().expand(len(y_true), len(ats))

    ats_rep = torch.tensor(data=ats, device=indices.device,
